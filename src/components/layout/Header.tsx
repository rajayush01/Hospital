      ...
      <nav className="absolute top-0 left-0 right-0 z-50 px-6 py-4">
        <div className="flex items-center justify-between mx-auto max-w-7xl">
          ...
          <div className="hidden lg:flex items-center w-full">
            ...
            >
              <img src={logo} alt="JK Hospitals" className="h-20 rounded-full" />
            </div>
...
const NavItem: React.FC<NavItemProps> = ({ label, onClick }) => (
  <button
    onClick={onClick}
    className="text-white hover:text-gray-300 flex items-center gap-2 text-sm xl:text-base"
  >
    {label}
...
const MobileNavItem: React.FC<NavItemProps> = ({ label, onClick }) => (
  <button
    onClick={onClick}
    className="text-white hover:text-gray-300 flex items-center justify-between py-2 px-3 rounded hover:bg-gray-800"
  >
    <span>{label}</span>
    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
